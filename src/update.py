...
def write_results():
    """
    Extract finished matches from fixtures.json and write data/results.json.
    Runs after fetch_fixtures() so it reflects the latest API data or cache.
    """
    fixtures_path = DATA / "fixtures.json"
    if not fixtures_path.exists():
        print("  Skipping: fixtures.json not found")
        return
    ...
    print(f"  ✓ {len(results)} result(s) written → {out_path.name}")
